...
const LEGACY_HASH_TO_V2_ROUTE: Record<string, string> = {
...
export default function DashboardPage() {
  ...
  const legacyEnabled = process.env.NEXT_PUBLIC_DASHBOARD_LEGACY_ENABLED === "true";
  ...
  useEffect(() => {
    ...
    router.replace(`/dashboard/overview${query}`);
  }, [legacyEnabled, router]);
  ...
  useEffect(() => {
    ...
    const mapped = LEGACY_HASH_TO_V2_ROUTE[hash];
    if (!mapped) {
      return;
    }
    const query = window.location.search || "";
  ...
  const dashboardMenuItems = useMemo<DashboardMenuItem[]>(
    () => [
      { id: "overview", label: "Overview", visible: true, href: "/dashboard/overview" },
      { id: "profile", label: "Profile", visible: true },
      { id: "api-keys", label: "API Keys", visible: true, href: "/dashboard/access/api-keys" },
      { id: "policy-simulator", label: "Policy Simulator", visible: true },
      { id: "mcp-usage", label: "MCP Usage", visible: true },
      { id: "audit-events", label: "Audit Events", visible: true, href: "/dashboard/control/audit-events" },
  ...
  if (!legacyEnabled) {
    return <main className="mx-auto max-w-5xl p-8">Redirecting to route-based dashboard...</main>;
  }
