...
export default function DashboardOverviewPage() {
  ...
  return (
    <section className="space-y-5">
      <h1 className="text-2xl font-bold tracking-tight leading-8">Overview</h1>
      <p className="text-[13px] leading-5 text-muted-foreground">KPI summary is loaded per page route and refreshed in page scope.</p>
        ...
        <div className="space-y-3">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <article className="ds-card p-4">
              ...
              <p className="mt-2 text-[30px] leading-9 font-semibold tabular-nums">{data.kpis.total_calls}</p>
            </article>
            <article className="ds-card p-4">
              <p className="text-xs text-muted-foreground">Success Rate</p>
              <p className="mt-2 text-[30px] leading-9 font-semibold tabular-nums text-chart-2">{(data.kpis.success_rate * 100).toFixed(1)}%</p>
            </article>
            <article className="ds-card p-4">
              <p className="text-xs text-muted-foreground">Fail Rate</p>
              <p className="mt-2 text-[30px] leading-9 font-semibold tabular-nums text-destructive">{(data.kpis.fail_rate * 100).toFixed(1)}%</p>
            </article>
            <article className="ds-card p-4">
              <p className="text-xs text-muted-foreground">Avg Latency</p>
              <p className="mt-2 text-[30px] leading-9 font-semibold tabular-nums">{Math.round(data.kpis.avg_latency_ms)} ms</p>
            </article>
            <article className="ds-card p-4">
              <p className="text-xs text-muted-foreground">P95 Latency</p>
              <p className="mt-2 text-[30px] leading-9 font-semibold tabular-nums">{Math.round(data.kpis.p95_latency_ms)} ms</p>
            </article>
            <article className="ds-card p-4">
              <p className="text-xs text-muted-foreground">Retry / Policy Block</p>
              <p className="mt-2 text-xl leading-7 font-semibold tabular-nums">
