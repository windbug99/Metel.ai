...
export default function DashboardMcpUsagePage() {
  ...
  return (
    <section className="space-y-5">
      <h1 className="text-2xl font-bold tracking-tight leading-8">MCP Usage</h1>
      <p className="text-[13px] leading-5 text-muted-foreground">Recent tool calls, 24h execution summary, and 7d usage trends.</p>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <article className="ds-card p-4">
          ...
          <p className="mt-1 text-[30px] leading-9 font-semibold tabular-nums">{toolCallsSummary?.calls_24h ?? 0}</p>
        </article>
        <article className="ds-card p-4">
          <p className="text-xs text-muted-foreground">Success (24h)</p>
          <p className="mt-1 text-[30px] leading-9 font-semibold tabular-nums text-chart-2">{toolCallsSummary?.success_24h ?? 0}</p>
        </article>
        <article className="ds-card p-4">
          <p className="text-xs text-muted-foreground">Fail (24h)</p>
          <p className="mt-1 text-[30px] leading-9 font-semibold tabular-nums text-destructive">{toolCallsSummary?.fail_24h ?? 0}</p>
        </article>
        <article className="ds-card p-4">
          <p className="text-xs text-muted-foreground">Fail Rate</p>
          <p className="mt-1 text-xl leading-7 font-semibold tabular-nums text-destructive">{((toolCallsSummary?.fail_rate_24h ?? 0) * 100).toFixed(1)}%</p>
        </article>
        <article className="ds-card p-4">
          <p className="text-xs text-muted-foreground">Blocked Rate</p>
          <p className="mt-1 text-xl leading-7 font-semibold tabular-nums text-chart-4">{((toolCallsSummary?.blocked_rate_24h ?? 0) * 100).toFixed(1)}%</p>
        </article>
        <article className="ds-card p-4">
          <p className="text-xs text-muted-foreground">Retryable Fail Rate</p>
          <p className="mt-1 text-xl leading-7 font-semibold tabular-nums text-primary">{((toolCallsSummary?.retryable_fail_rate_24h ?? 0) * 100).toFixed(1)}%</p>
        </article>
      </div>
      ...
      <div className="ds-card space-y-2 p-4">
        <p className="text-sm font-semibold">Top Failure Codes (24h)</p>
      ...
      <div className="ds-card space-y-2 p-4">
        <p className="text-sm font-semibold">Recent Tool Calls</p>
      ...
      <div className="ds-card p-4">
        <div className="mb-3 flex items-center justify-between gap-2">
          <p className="text-sm font-semibold">Usage Trends (7d)</p>
